...
export async function GET(req: Request) {
  ...
  return NextResponse.json({ ok: true, drafts: listDrafts(portalId, "list_draft") });
}
...
export async function POST(req: Request) {
  ...
  const name = body.name || String(body.spec.name) || "Untitled List";
  const draft = saveDraft(body.portalId, "list_draft", name, body.spec);
