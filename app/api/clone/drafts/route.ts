...
export async function GET(req: Request) {
  ...
  return NextResponse.json({ ok: true, drafts: listDrafts(portalId, "clone_draft") });
}
...
export async function POST(req: Request) {
  ...
  const name = body.name || String(body.spec.name) || "Untitled Clone Config";
  const draft = saveDraft(body.portalId, "clone_draft", name, body.spec);
