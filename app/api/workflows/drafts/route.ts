...
export async function GET(req: Request) {
  ...
  return NextResponse.json({ ok: true, drafts: listDrafts(portalId, "workflow_draft") });
}
...
export async function POST(req: Request) {
  ...
  const name = body.name || String(body.spec.name) || "Untitled Workflow";
  const draft = saveDraft(body.portalId, "workflow_draft", name, body.spec);
